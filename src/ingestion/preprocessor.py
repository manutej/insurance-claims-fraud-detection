...
class ClaimPreprocessor:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.encoding_strategy = self.config.get('encoding_strategy', 'onehot')  # 'onehot', 'label', 'target'
    ...
    def preprocess_claims(self, claims: List[BaseClaim]) -> pd.DataFrame:
        ...
        self.feature_columns = [col for col in df.columns if col not in ['claim_id', 'fraud_indicator']]
        self.is_fitted = True
        ...
        for claim in claims:
            ...
            if isinstance(claim, MedicalClaim):
                claim_dict.update({
                    ...
                    'primary_diagnosis': claim.diagnosis_codes[0] if claim.diagnosis_codes else None,
                    'primary_procedure': claim.procedure_codes[0] if claim.procedure_codes else None,
                    'service_location': claim.service_location,
                    'rendering_hours': float(claim.rendering_hours) if claim.rendering_hours else None,
    ...
    def _extract_basic_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['amount_bin'] = pd.cut(df['billed_amount'],
    ...
    def _extract_provider_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        provider_stats = df.groupby('provider_npi').agg({
            'claim_id': 'count',
            'billed_amount': ['mean', 'std', 'sum'],
            'fraud_indicator': 'mean'
        }).reset_index()

        provider_stats.columns = ['provider_npi', 'provider_claim_count',
                                 'provider_avg_amount', 'provider_amount_std',
                                 'provider_total_amount', 'provider_fraud_rate']
    ...
    def _handle_missing_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if numerical_cols:
            if 'numerical_imputer' not in self.imputers:
                ...
                df[numerical_cols] = self.imputers['numerical_imputer'].fit_transform(df[numerical_cols])
            else:
                df[numerical_cols] = self.imputers['numerical_imputer'].transform(df[numerical_cols])
        ...
        if categorical_cols:
            if 'categorical_imputer' not in self.imputers:
                ...
                df[categorical_cols] = self.imputers['categorical_imputer'].transform(df[categorical_cols])
        ...
        for col in categorical_cols:
            if col in df.columns:
                ...
                if encoder_key not in self.encoders:
                    self.encoders[encoder_key] = OneHotEncoder(sparse=False, drop='first', handle_unknown='ignore')
                    encoded = self.encoders[encoder_key].fit_transform(df[[col]])
                else:
                    encoded = self.encoders[encoder_key].transform(df[[col]])

                # Create column names
                feature_names = [f'{col}_{cat}' for cat in self.encoders[encoder_key].categories_[0][1:]]
