...
class RuleEngine:
            ...
            if 'rules' in config:
                for rule_name, rule_config in config['rules'].items():
                    if rule_name in self.rules:
                        self.rules[rule_name].threshold = rule_config.get('threshold', self.rules[rule_name].threshold)
                        self.rules[rule_name].weight = rule_config.get('weight', self.rules[rule_name].weight)
                        self.rules[rule_name].enabled = rule_config.get('enabled', self.rules[rule_name].enabled)
    ...
    def analyze_claim(self, claim: Dict[str, Any], context_claims: List[Dict[str, Any]] = None) -> Tuple[List[RuleResult], float]:
        """
    ...
    def _apply_rule(self, rule: FraudRule, claim: Dict[str, Any], context_claims: List[Dict[str, Any]]) -> RuleResult:
            ...
            if proc_code in high_complexity_procedures:
                for diag_code in diagnosis_codes:
                    if diag_code in simple_diagnoses:
                        evidence.append(f"High complexity procedure {proc_code} for simple diagnosis {diag_code}")
                        score += 0.3
    ...
    def _check_phantom_billing_schedule(self, rule: FraudRule, claim: Dict[str, Any]) -> RuleResult:
        ...
        try:
            ...
            if day_of_week in ['Saturday', 'Sunday'] and service_location != '23':  # Not emergency room
    ...
    def _check_unbundling(self, rule: FraudRule, claim: Dict[str, Any], context_claims: List[Dict[str, Any]]) -> RuleResult:
        ...
        if context_claims:
            same_day_claims = [
                ...
                if (c.get('date_of_service') == claim_date and
                    c.get('patient_id') == patient_id and
                    c.get('provider_id') == provider_id and
                    c.get('claim_id') != claim.get('claim_id'))
            ]
            ...
            for other_claim in same_day_claims:
                ...
                if common_procedures:
                    evidence.append(f"Duplicate procedures across claims: {list(common_procedures)}")
                    score += 0.5
    ...
    def _check_staged_accident(self, rule: FraudRule, claim: Dict[str, Any], context_claims: List[Dict[str, Any]]) -> RuleResult:
        ...
        if claim_type == 'auto' or any(code in auto_injury_codes for code in diagnosis_codes):
            ...
            if context_claims:
                similar_accidents = [
                    c for c in context_claims
                    if (c.get('claim_type') == 'auto' and
                        len(set(c.get('diagnosis_codes', [])) & set(diagnosis_codes)) > 0)
                ]
    ...
    def _check_kickback_scheme(self, rule: FraudRule, claim: Dict[str, Any], context_claims: List[Dict[str, Any]]) -> RuleResult:
        ...
        if context_claims:
            ...
            if referral_pattern['concentration'] > self.thresholds['referral_concentration_threshold']:
                evidence.append(f"High referral concentration: {referral_pattern['concentration']:.2f}")
                score += 0.4
    ...
    def _check_billing_frequency(self, rule: FraudRule, claim: Dict[str, Any]) -> RuleResult:
        ...
        provider_claims_today = len([
            c for c in self.provider_history[provider_id]
        ...
        patient_visits_today = len([
            c for c in self.patient_history[patient_id]
            if c.get('date_of_service') == date_of_service
        ])
    ...
    def _check_amount_anomaly(self, rule: FraudRule, claim: Dict[str, Any]) -> RuleResult:
        ...
        if expected_amount > 0 and billed_amount > expected_amount * 2:
            evidence.append(f"Amount ${billed_amount} significantly exceeds expected ${expected_amount}")
            score += 0.4
    ...
    def _analyze_referral_patterns(self, provider_id: str, context_claims: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    def _has_circular_referrals(self, provider_id: str, context_claims: List[Dict[str, Any]]) -> bool:
        """Check for circular referral patterns."""
