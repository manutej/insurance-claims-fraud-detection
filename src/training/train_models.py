...
class ModelTrainingPipeline:
    ...
    def load_data(self, data_path: str = None, claims_data: List[Dict[str, Any]] = None) -> pd.DataFrame:
        """
    ...
    def prepare_data(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        logger.info(f"Dataset composition: {fraud_count} fraud, {total_count - fraud_count} legitimate")
    ...
    def engineer_features(self, train_df: pd.DataFrame, val_df: pd.DataFrame,
    ...
    def select_features(self, X_train: pd.DataFrame, y_train: pd.Series,
        ...
        logger.info(f"Selected {len(selected_features)} features using {self.config.feature_selection_method}")

        return X_train[selected_features], X_val[selected_features], X_test[selected_features]

    def train_models(self, X_train: pd.DataFrame, y_train: pd.Series,
                    X_val: pd.DataFrame, y_val: pd.Series) -> Dict[str, ModelPerformance]:
        """
    ...
    def evaluate_final_performance(self, X_test: pd.DataFrame, y_test: pd.Series) -> Dict[str, float]:
        """
    ...
    def run_full_pipeline(self, data_path: str = None, claims_data: List[Dict[str, Any]] = None) -> TrainingResults:
        """
        ...
        try:
            ...
            best_model = max(model_performances.items(),
                           key=lambda x: x[1].accuracy if hasattr(x[1], 'accuracy') else 0)[0]

            # Check if targets are met
            meets_targets = (
                final_performance['accuracy'] >= self.config.target_accuracy and
                final_performance['false_positive_rate'] <= self.config.target_false_positive_rate
            )
    ...
    def _generate_recommendations(self, final_performance: Dict[str, float],
                                model_performances: Dict[str, ModelPerformance]) -> List[str]:
        """Generate recommendations based on training results."""
        recommendations = []

        # Performance recommendations
        if final_performance['accuracy'] < self.config.target_accuracy:
            recommendations.append(f"Accuracy ({final_performance['accuracy']:.3f}) below target ({self.config.target_accuracy:.3f}). Consider feature engineering or ensemble methods.")

        if final_performance['false_positive_rate'] > self.config.target_false_positive_rate:
            recommendations.append(f"False positive rate ({final_performance['false_positive_rate']:.3f}) above target ({self.config.target_false_positive_rate:.3f}). Adjust classification thresholds.")

        if final_performance['avg_processing_time_ms'] > self.config.target_processing_time_ms:
            recommendations.append(f"Processing time ({final_performance['avg_processing_time_ms']:.1f}ms) above target. Consider feature reduction or model optimization.")

        # Model-specific recommendations
        best_accuracy = max([p.accuracy for p in model_performances.values() if hasattr(p, 'accuracy')])
        if best_accuracy < 0.90:
            recommendations.append("Overall model performance is low. Consider additional feature engineering or more training data.")
    ...
    def save_results(self) -> None:
        ...
        if self.anomaly_detector:
            self.anomaly_detector.save_detectors(os.path.join(self.config.output_directory, 'anomaly_detectors'))
        ...
        if self.training_results:
            ...
            with open(os.path.join(self.config.output_directory, 'training_results.json'), 'w') as f:
        ...
        if self.config.save_training_data:
            training_summary = {
                ...
                'fraud_rate': self.training_data['fraud_indicator'].mean() if self.training_data is not None else 0,
    ...
    def generate_plots(self, X_test: pd.DataFrame, y_test: pd.Series) -> None:
        ...
        try:
            ...
            if self.training_results and self.training_results.model_performances:
                ...
                ax2 = ax1.twinx()
                ax2.bar(x + width/2, fprs, width, label='False Positive Rate', alpha=0.8, color='red')
                ax2.set_ylabel('False Positive Rate')

                # Target lines
                ax1.axhline(y=self.config.target_accuracy, color='blue', linestyle='--', alpha=0.7, label='Target Accuracy')
                ax2.axhline(y=self.config.target_false_positive_rate, color='red', linestyle='--', alpha=0.7, label='Target FPR')
    ...
    def generate_training_report(self) -> str:
        ...
        report += f"Meets Performance Targets: {'✓' if self.training_results.meets_targets else '✗'}\n\n"
        ...
        if self.training_results.feature_importance:
            ...
            sorted_features = sorted(self.training_results.feature_importance.items(),
