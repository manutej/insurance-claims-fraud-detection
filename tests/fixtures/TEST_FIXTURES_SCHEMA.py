...
class TestClaimFactory:
    ...
    def create_valid_claim(
        ...
        return CompleteTestClaim(
            ...
            expected_result=ExpectedDetectionResult(
                should_detect_fraud=False, expected_score_range=(0.0, 0.3), notes="Legitimate claim"
            ),
        )

    @staticmethod
    def create_upcoding_claim(
        claim_id: str = "CLM-TEST-FRAUD-001", severity: FraudSeverity = FraudSeverity.OBVIOUS
    ) -> CompleteTestClaim:
...
def generate_test_batch(
    ...
    for i in range(num_legitimate):
        ...
        if i < num_incomplete:
            ...
            incomplete_claims.append(factory.create_incomplete_claim(claim_id, completeness))
    ...
    for i in range(num_fraud):
        ...
        fraud_type = random.choices(
            list(fraud_type_distribution.keys()), weights=list(fraud_type_distribution.values())
        )[0]
...
def pytest_fixtures():
    ...
    return {
        ...
        "test_batch_balanced": generate_test_batch(
            batch_id="BAL001", total_claims=100, fraud_rate=0.5, incomplete_rate=0.0
        ),
        "test_batch_realistic": generate_test_batch(
            batch_id="REAL001", total_claims=1000, fraud_rate=0.12, incomplete_rate=0.3
        ),
    }
...
class TestResultValidator:
    ...
    def validate_fraud_detection_result(
        ...
        if actual_fraud_detected != expected.should_detect_fraud:
            return (
                False,
                f"Expected fraud_detected={expected.should_detect_fraud}, got {actual_fraud_detected}",
            )

        if not (
            expected.expected_score_range[0]
            <= actual_fraud_score
            <= expected.expected_score_range[1]
        ):
            return (
                False,
                f"Score {actual_fraud_score} outside expected range {expected.expected_score_range}",
            )
        ...
        if expected.expected_fraud_types:
            if not set(actual_fraud_types).intersection(set(expected.expected_fraud_types)):
                return (
                    False,
                    f"Expected fraud types {expected.expected_fraud_types}, got {actual_fraud_types}",
                )
    ...
    def validate_enrichment_result(
        ...
        if "diagnosis_codes" in enriched_claim and ground_truth.diagnosis_codes:
            ...
            if not enriched_diag.intersection(ground_truth_diag):
                return (
                    False,
                    f"Enriched diagnoses {enriched_diag} don't match ground truth {ground_truth_diag}",
                )
