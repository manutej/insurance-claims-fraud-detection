...
from src.ingestion import (
    ClaimDataLoader, DataLoaderConfig, ClaimValidator, ClaimPreprocessor
)
...
@cli.command()
@click.option('--claim-types', '-t', multiple=True, help='Claim types to load (medical, pharmacy, no_fault)')
@click.option('--batch-size', '-b', default=1000, help='Batch size for processing')
@click.option('--validate', '-val', is_flag=True, default=True, help='Validate claims during loading')
...
@click.option('--format', '-f', type=click.Choice(['json', 'csv', 'parquet']), default='json', help='Output format')
@click.pass_context
def load(ctx, claim_types, batch_size, validate, preprocess, output, format):
    ...
    try:
        ...
        with Progress(
            ...
            console.print(f"Found {file_summary['total_files']} files ({file_summary['total_size_mb']:.1f} MB)")
...
def stream(ctx, claim_types, chunk_size, max_chunks, validate):
    ...
    try:
        ...
        with Progress(
            ...
            task = progress.add_task("Streaming claims...", total=max_chunks if max_chunks > 0 else None)
            ...
            for chunk in loader.stream_claims(
                ...
                if validate:
                    ...
                    if validation_result.error_count > 0:
                        console.print(f"[yellow]Validation errors in chunk {chunk_count}: {validation_result.error_count}[/yellow]")
        ...
        console.print(f"[bold green]Streaming complete:[/bold green] {chunk_count} chunks, {total_claims} total claims")
...
def info(ctx):
    ...
    try:
        ...
        for file_type, count in summary['files_by_type'].items():
            type_size = sum(
                detail['size_mb'] for detail in summary['file_details']
...
def _display_validation_results(result: ProcessingResult, detailed: bool = False):
    ...
    table.add_row("Errors", str(result.error_count), style="red" if result.error_count > 0 else "green")
    table.add_row("Warnings", str(result.warnings_count), style="yellow" if result.warnings_count > 0 else "green")
...
def _save_batch_output(batch: ClaimBatch, output_path: str, format: str, preprocess: bool, preprocessor: Optional[ClaimPreprocessor]):
...
def stream_async(ctx, claim_types, chunk_size, max_chunks):
    ...
    async def async_stream():
        ...
        console.print(f"[bold green]Async streaming complete:[/bold green] {chunk_count} chunks, {total_claims} total claims")
